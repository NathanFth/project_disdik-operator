...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Ubah username jadi email (operator.sd -> operator.sd@example.com)
      let email = username;
      if (!email.includes("@")) {
      ...
      // Simpan ke localStorage untuk kebutuhan UI (bukan untuk security)
      ...
      // ====== Redirect dengan dukungan ?next= ======
      // Contoh path login: /divisi-sd/login -> ['', 'divisi-sd', 'login']
      const division = pathname.split("/")[1] || "divisi-sd";
      const root = `/${division}`;
      const next = searchParams.get("next");

      const target = next && next.startsWith(root) ? next : `${root}/dashboard`;

      router.replace(target); // URL di target bersih, tanpa ?next
    } catch (err) {
